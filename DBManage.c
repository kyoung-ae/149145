...
void backup() { // CPS.db 파일을 CPS_backup.db 파일로 복사(백업)
    system("sqlite3 CPS.db .dump | sqlite3 CPS_backup.db"); // CPS.db 파일을 CPS_backup.db 파일로 복사(백업)
    //system("sqlite3 CPS.db .dump); // CPS.db 백업
    system("clear"); // CPS.db 백업 sql 내용일 화면에 노출되지 않게 지움 (이유 : pwd 까지 출력되기 때문)
    //printf("CPS_backup.db 파일을 확인해보세요.\n");
    //printf("지금까지의 작업이 백업됐습니다.\n");
    //printf("DB 시작화면으로 돌아갑니다.\n");
    //login(); // DBLogin.c
    printf("지금까지의 DB 내용이 CPS_backu.db 파일로 백업됐습니다.\n");
}
...
int checkIDPWD(char id[]) { // 입력한 id를 매개변수로 받아서 id&pwd 확인 (성공하면 1을 반환)
    ...
    //strncpy(sql_checkidpwd, "SELECT pwd FROM ADMIN WHERE id = '", 34);
